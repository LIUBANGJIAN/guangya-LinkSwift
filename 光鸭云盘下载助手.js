...
// @description       光鸭云盘文件下载地址获取工具
// @description:zh-CN 光鸭云盘文件下载地址获取工具
...
(function linkSwift($) {
	...
	const config = {
		...
		$guangya: {
			...
			convert: {
				aria2: `--header "Referer:https://www.guangyapan.com/"`,
				curl: `-H "Referer:https://www.guangyapan.com/"`,
				bitcomet: `referrer=${encodeURIComponent("https://www.guangyapan.com/")}`
			}
		},
		$quark: {
	...
	const base = {
		...
		async sendLinkToBitcomet(link, filename, headers) {
			...
			return this.sendLinkToBitcomet.lock = this.sendLinkToBitcomet.lock.then(async () => {
				...
				console.log("【LinkSwift】BitComet: 发送任务到", url);
				console.log("【LinkSwift】BitComet: 文件名:", filename);
				console.log("【LinkSwift】BitComet: 链接长度:", link.length);
				console.log("【LinkSwift】BitComet: 请求数据:", Object.fromEntries(data));
				try {
					...
					console.log("【LinkSwift】BitComet: 响应结果:", res);
					if (res && res.includes("Add task failed!")) {
						console.log("【LinkSwift】BitComet: 任务添加失败");
						return "fail";
					} else {
						console.log("【LinkSwift】BitComet: 任务添加成功");
						return "success";
					}
				} catch (e) {
					console.error("【LinkSwift】BitComet: 请求异常:", e);
					return "fail";
				}
			});
		},
	...
	const $guangya = {
		addPageListener() {
			...
			$doc.on("click", ".listener-bitcomet-download", async function (e) {
				...
				target.find(".pl-loading").remove();
				target.prepend(base.createLoading());
				const res = await base.sendLinkToBitcomet(target.data("link"), target.data("filename"), { "referrer": "https://www.guangyapan.com/", "checkboxCustomHeadersForMirrors": "on" });
				if (res === "success") {
					target.removeClass("pl-btn-danger").html("发送成功啦!快去看看吧~").animate({ opacity: "0.5" }, "slow");
				} else {
					target.addClass("pl-btn-danger").text("发送失败，检查一下您的配置信息哦!").animate({ opacity: "0.5" }, "slow");
		...
		async getFileUrl(item, index, token, shareToken) {
			try {
				...
				console.error("【LinkSwift】Guangya getFileUrl error:", e);
				return { index, downloadUrl: "获取下载地址失败，可能是网络问题或登录状态失效，请刷新页面后重试~" };
			}
		},
		async getLink() {
			const selects = this.getSelectedList();
		...
		getSelectedList() {
			try {
				...
				// 方法1: 尝试从所有class包含file的div中查找React对象
				console.log("【LinkSwift】Guangya: 尝试从class包含file的元素查找...");
				const fileElements = document.querySelectorAll(`div[class*="file"]`);
				console.log("【LinkSwift】Guangya: 找到", fileElements.length, "个class包含file的元素");
				
				for (const element of fileElements) {
					const reactObj = base.findReact(element, 0);
					if (reactObj) {
						let fileData = null;
						if (reactObj.pendingProps) fileData = reactObj.pendingProps;
						else if (reactObj.props) fileData = reactObj.props;
						else if (reactObj.memoizedProps) fileData = reactObj.memoizedProps;
						
						// 检查是否同时包含dataSource和selectedItems
						if (fileData && fileData.selectedItems && Array.isArray(fileData.selectedItems) && fileData.dataSource && Array.isArray(fileData.dataSource)) {
							console.log("【LinkSwift】Guangya: 找到selectedItems和dataSource");
							console.log("【LinkSwift】Guangya: selectedItems内容:", fileData.selectedItems);
							console.log("【LinkSwift】Guangya: dataSource长度:", fileData.dataSource.length);
							
							const selectedIds = fileData.selectedItems;
							console.log("【LinkSwift】Guangya: 选中的文件ID数量:", selectedIds.length);
							
							// 根据ID从dataSource中匹配文件数据
							for (const fileId of selectedIds) {
								// 如果fileId是字符串，说明是文件ID，需要从dataSource中查找
								if (typeof fileId === 'string') {
									const fileItem = fileData.dataSource.find(item => 
										item.id === fileId || 
										item.fileId === fileId || 
										item.file_id === fileId
									);
									if (fileItem) {
										console.log("【LinkSwift】Guangya: 找到匹配的文件:", fileItem.fileName || fileItem.name);
										selectedList.push({ ...fileItem });
									} else {
										console.log("【LinkSwift】Guangya: 未找到ID为", fileId, "的文件");
									}
								} else if (fileId.fileId || fileId.file_id || fileId.id) {
									// 如果fileId是对象，直接使用
									selectedList.push({ ...fileId });
								}
							}
							
							console.log("【LinkSwift】Guangya: 从dataSource匹配到", selectedList.length, "个文件");
							return base.clone(selectedList);
						}
					}
				}
				
				console.log("【LinkSwift】Guangya: 最终选中", selectedList.length, "个文件");
				console.log("【LinkSwift】Guangya: 选中文件详情:", selectedList);
				return base.clone(selectedList);
			} catch (e) {
				console.error("【LinkSwift】Guangya getSelectedList error:", e);
				return [];
			}
		},
		detectPage() {
